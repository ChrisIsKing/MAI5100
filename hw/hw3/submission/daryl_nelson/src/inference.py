...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            currentFactorsNotToJoin, joinedFactor = joinFactorsByVariable(factors, variable)

            if len(joinedFactor.unconditionedVariables()) > 1:
                joinedFactor = eliminate(joinedFactor, variable)
                currentFactorsNotToJoin.append(joinedFactor)

            factors = currentFactorsNotToJoin

        factor_product = factors.pop()
        for factor in factors:
            factor_product = joinFactors([factor_product, factor])

        factor_product = normalize(factor_product)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            return busters.getObservationProbability(noisyDistance, manhattan_dist)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            ...
            newPositionDistance = self.getPositionDistribution(gameState, position)

            for newPosition, transitionProbability in newPositionDistance.items():
                newBeliefs[newPosition] += probability * transitionProbability
