...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Add all variables to the Bayes Net
    ...
    max_true_distance = (X_RANGE - 1) + (Y_RANGE - 1)
    max_observed_distance = max_true_distance + MAX_NOISE
    
    observations = []
    for dist in range(max_observed_distance + 1):  # 0 to max_observed_distance inclusive
        observations.append(dist)
    
    variableDomainsDict[OBS0] = observations
    variableDomainsDict[OBS1] = observations
    
    # Add edges according to the Bayes Net structure
    # Each observation depends on Pacman's position and the corresponding ghost's position
    edges = [
        (PAC, OBS0),      # Pacman position affects Observation0
        (GHOST0, OBS0),   # Ghost0 position affects Observation0
        (PAC, OBS1),      # Pacman position affects Observation1
        (GHOST1, OBS1)    # Ghost1 position affects Observation1
    ]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            # Find factors that contain this variable and those that don't
            ...
            if len(joinedFactor.unconditionedVariables()) == 1:
                # Don't add the joined factor back to the list - effectively discarding it
                # The result of eliminating the last unconditioned variable would be 1
                pass
            else:
                # Eliminate the variable from the joined factor
                eliminatedFactor = eliminate(joinedFactor, eliminationVariable)
                # Add the eliminated factor back to our list
                currentFactorsList.append(eliminatedFactor)
        
        if len(currentFactorsList) == 0:
            # Edge case: no factors left (shouldn't happen in normal cases)
            raise ValueError("No factors remaining after elimination")
        elif len(currentFactorsList) == 1:
            # Only one factor left - this is our result
            fullJointOverQueryAndEvidence = currentFactorsList[0]
        else:
            # Multiple factors left - join them all
            fullJointOverQueryAndEvidence = joinFactors(currentFactorsList)
        
        # This converts P(query, evidence) to P(query | evidence)
        queryConditionedOnEvidence = normalize(fullJointOverQueryAndEvidence)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            # Use modulo to ensure even distribution across legal positions
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Step 1: Calculate weights for each particle position
        weights = DiscreteDistribution()
        
        for particle in self.particles:
            # Calculate how well this particle explains the observation
            weight = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            weights[particle] += weight
        
        # Step 2: Handle the special case where all particles have zero weight
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return
        ...
        # Step 4: Replace old particles with new resampled particles
