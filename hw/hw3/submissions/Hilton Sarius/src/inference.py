...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # Define agents and their relationships
    agents = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    paths = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]

    # Initialize domain dictionary
    domain = {str(e): [] for e in agents}

    # Generate all possible positions in the grid
    positions = [(x, y) for y in range(Y_RANGE) for x in range(X_RANGE)]
    for entity in [PAC, GHOST0, GHOST1]:
        domain[entity] = positions.copy()

    # Compute all possible noisy distances
    noise_range = range(-MAX_NOISE, MAX_NOISE + 1)
    noisy_distances = {
        abs(a[0] - b[0]) + abs(a[1] - b[1]) + n
        for a in domain[PAC]
        for b in domain[PAC]
        for n in noise_range
        if abs(a[0] - b[0]) + abs(a[1] - b[1]) + n >= 0
    }

    # Assign noisy distances to WALL domains
    domain[OBS0] = domain[OBS1] = noisy_distances
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
